...
const GITLAB_GROUP = 'http://git.jtjr.com/groups/h5_webtools_grp';
const PRIVATE_TOKEN = 'pT4s5vecEPSTbRGDUBJy';
...
function getGroupsData() {
  return new Promise((resolve, reject) => {
    const match = GITLAB_GROUP.match(/^(http|https):\/\/(\S+)\/groups\/(\S+)$/i);
    ...
    const options = {
      ...
      path: `/api/v3/groups/${match[3]}?private_token=${PRIVATE_TOKEN}`,
      headers: {
        'User-Agent': 'jfet-gui'
      },
      method: 'GET'
